...
private:
  ...
  // Packet map that stores IP address and corresponding ARP request time and 
  // queue of datagram which do not receive replies
  std::unordered_map<uint32_t, std::pair<size_t, std::queue<InternetDatagram>>> queue_table_;

  // Ready-to-be-sent queue
  std::queue<EthernetFrame> frame_queue_;

  // ARP cache table, stores the ip address that sent the request and the corresponding 
  // the creating time in ms and MAC address
  std::unordered_map<uint32_t, std::pair<size_t, EthernetAddress>> arp_table_;

  // Current time
  size_t curr_time_ = 0;

  // Erase all expired items in map, determined by expire_time.
  void erase_expired_item(auto& map, size_t expire_time);
